<template>
	<div>
		...
		<el-tabs v-model="activeName" class="mt20">
			<el-tab-pane label="店铺体检报告购买" name="first">
				<el-form ref="form" :rules="rules" :model="form" label-width="80px" class="mt10" label-position="left">
					...
					<el-form-item label="店铺邮箱" prop="email">
						<el-input v-model="form.email" style="width: 217px;" placeholder="请输入店铺邮箱"></el-input>
					</el-form-item>
					...
					<el-form-item label="报告类型" prop="name">
						<el-card style="width: 500px">
							...
							<p class="info">店铺体检报告，未雨绸缪，预知风险才能做好防范准备。</p>
							<p class="txt-r">
			...
			<el-tab-pane label="购买记录" name="second" class="mt10">
				...
				<el-table :data="tableData" @selection-change="selsChange" v-loading="listLoading" style="width: 100%" id="tableData"
				 ref='tableData'>
					...
					<el-table-column prop="ShopEmail" label="店铺邮箱" align="center"></el-table-column>
					...
					<el-table-column prop="ShopType" label="报告截图" align="center">
						<template slot-scope="scope">
							<img style="width: 40px;height: 40px;cursor: pointer;" v-show="scope.row.ShopType" :src="$IMGURL+scope.row.ShopType"
		...
		<!-- 示例文档 -->
		<el-dialog title="店铺体检报告样例" :visible.sync="helpModal" width="50%">
			<img src="../../assets/image/store_checkup_report.png" />
			<div slot="footer" class="dialog-footer txt-c">
				<el-link type="primary" class="fz16" :underline="false" href="/document/店铺体检报告样例.jpg">下载样例</el-link>
			</div>
		</el-dialog>
	...
	export default {
		...
		methods: {
				...
				_this.$refs.form.validate((valid) => {
					if (valid) {
						...
						let params = {
							...
							ShopEmail: _this.form.email,
							ShopId: _this.form.shopId,
							Price: _this.form.price
						}
						shopCheckAdd(params).then(res => {
			...
			showImage(index, row) {
				...
				_this.bigImageUrl = this.$IMGURL + row.ShopType
			},
